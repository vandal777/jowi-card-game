...
function App() {
  ...
  const selectRandomCard = () => {
    const randomSelectedCard = Math.floor(
      Math.random() * (cards.length - 1 - 1 + 1) + 1
    );

    const valueToRemove = cards[randomSelectedCard];

    const filteredItems = cards.filter((item) => item !== valueToRemove);
